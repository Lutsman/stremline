...
if ( ! class_exists( 'streamline_Instagram_Widget' ) ) {

	class streamline_Instagram_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of photos from Instagram network.', 'streamline' );
			$this->widget_id          = 'streamline_widget_instagram';
			$this->widget_name        = esc_html__( 'Instagram', 'streamline' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'Instagram', 'streamline' ),
					'label' => esc_html__( 'Title', 'streamline' ),
				),
				'tag' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Hashtag (enter without `#` symbol)', 'streamline' ),
				),
				'image_counter' => array(
					'type'       => 'stepper',
					'value'      => '6',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Number of photos', 'streamline' ),
				),
				'display_caption' => array(
					...
					'options' => array(
						'display_caption_check' => esc_html__( 'Caption', 'streamline' ),
					),
				),
				'display_date' => array(
					'type'  => 'checkbox',
					...
					'options' => array(
						'display_date_check' => esc_html__( 'Date', 'streamline' ),
					),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( empty( $instance['tag'] ) ) {
				return print $args['before_widget'] . esc_html__( 'Please, enter #hashtag.', 'streamline' ) . $args['after_widget'];
			}
			...
			if ( ! $photos ) {
				return print $args['before_widget'] . esc_html__( 'No photos. Maybe you entered a invalid hashtag.', 'streamline' ) . $args['after_widget'];
			}

			$template = locate_template( 'inc/widgets/tm-instagram-widget/views/instagram.php', false, false );

			printf( '<div class="%s">',
				join( ' ', apply_filters( 'streamline_instagram_widget_wrapper_class', array( 'instagram__items' ) ) )
			);
		...
		public function get_photos( $data, $img_counter, $config ) {
			 ...
			 * @var string
			 */
			$order = apply_filters( 'streamline_instagram_widget_order', 'media' );
			...
			foreach ( $nodes as $photo ) {
				...
				if ( in_array( 'caption', $config ) ) {
					$_photo['caption'] = wp_html_excerpt(
						$photo['caption'],
						apply_filters( 'streamline_instagram_widget_caption_length', 10 ),
						apply_filters( 'streamline_instagram_widget_caption_more', '&hellip;' )
					);
				}
		...
		public function get_tags_url() {
			return apply_filters( 'streamline_instagram_widget_get_tags_url', $this->service_url . 'explore/tags/%s/' );
		}
		...
		public function get_post_url() {
			return apply_filters( 'streamline_instagram_widget_get_post_url', $this->service_url . 'p/%s/' );
		}
	...
	add_action( 'widgets_init', 'streamline_register_instagram_widget' );
	function streamline_register_instagram_widget() {
		register_widget( 'streamline_Instagram_Widget' );
	}
}
