...
class streamline_News_Smart_Box_Widget extends Cherry_Abstract_Widget {

	/**
	 * Constructor
	 */
	public function __construct() {
		$this->widget_cssclass = 'streamline widget_news_smart_box';
		...
		$this->widget_name = esc_html__( 'New Smart Box Widget', 'streamline' );
		$this->settings = array(
			'title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'News Smart Box', 'streamline' ),
			...
			'layout_type' => array(
				...
				'options'			=> array(
					'layout_type_1' => array(
						'label'		=> esc_html__( 'Layout type 1', 'streamline' ),
						'img_src'	=> STREAMLINE_THEME_URI . '/assets/images/admin/widgets/news-smart-box/news-smart-box-layout-1.svg',
					),
					'layout_type_2' => array(
						'label'		=> esc_html__( 'Layout type 2', 'streamline' ),
						'img_src'	=> STREAMLINE_THEME_URI . '/assets/images/admin/widgets/news-smart-box/news-smart-box-layout-2.svg',
					),
					'layout_type_3' => array(
						'label'		=> esc_html__( 'Layout type 3', 'streamline' ),
						'img_src'	=> STREAMLINE_THEME_URI . '/assets/images/admin/widgets/news-smart-box/news-smart-box-layout-3.svg',
					),
				),
				'label'				=> esc_html__( 'Choose layout type', 'streamline' ),
			),
			'terms_type' => array(
				'type'				=> 'radio',
				'value'				=> 'category',
			...
			'categories' => array(
				...
				'label' => esc_html__( 'Select category', 'streamline' ),
				'multiple' => true,
				'placeholder' => esc_html__( 'Select categories', 'streamline' ),
			),
			'current_tag' => array(
				'type' => 'select',
				'value' => '',
				...
				'options' => false,
				'label' => esc_html__( 'Select tag', 'streamline' ),
			...
			'trim_title_words' => array(
				...
				'min_value' => 1,
				'step_value' => 1,
				'label'  => esc_html__( 'Title words trimmed count', 'streamline' ),
			),
			...
			'date_visibility' => array(
				...
				'label' => esc_html__( 'Display date', 'streamline' ),
			),
			'author_visibility' => array(
				'type' => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display author', 'streamline' ),
			),
			'comment_visibility' => array(
				'type' => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display comments', 'streamline' ),
			),
		);
	...
	public function get_smart_box_loop( $posts_query ) {
		...
		if ( $posts_query->have_posts() ) {
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				...
				$placeholder_args = apply_filters( 'streamline_news_smart_box_placeholder_args',
					array(
						'width'			=> 560,
						'height'		=> 350,
						'background'	=> '000',
				...
				$image_size = apply_filters( 'streamline_carousel_image_size', 'streamline-thumb-536-411' );
				...
				$full_view_dir = locate_template( 'inc/widgets/tm-news-smart-box-widget/views/tm-news-smart-box-full-view.php' );
	...
	public function get_image( $id, $size, $placeholder_attr, $only_url = false ) {
		...
		$default_placeholder_attr = apply_filters( 'streamline_news_smart_box_placeholder_default_args',
			array(
				'width'			=> 900,
				'height'		=> 500,
				'background'	=> '000',
		...
		if ( has_post_thumbnail( $id ) ) {
			...
			$attachment_image = wp_get_attachment_image_src( $thumbnail_id, 'streamline-thumb-536-411' );
...
add_action( 'widgets_init', 'streamline_register_news_smart_box_widgets' );
function streamline_register_news_smart_box_widgets() {
	register_widget( 'streamline_News_Smart_Box_Widget' );
}