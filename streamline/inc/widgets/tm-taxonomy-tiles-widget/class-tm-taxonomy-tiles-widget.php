...
if ( !class_exists( 'streamline_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class streamline_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name			= esc_html__( 'Taxonomy Tiles Widget', 'streamline' );
			$this->widget_description	= esc_html__( 'This widget displays images from taxonomy.', 'streamline' );
			...
			$this->settings = array(
				...
				'category'=> array(
					...
					'label'				=> esc_html__( 'Select category to show', 'streamline' ),
				),
				'post_tag' => array(
					'type'				=> 'select',
					'multiple'			=> true,
				...
				'description_length' => array(
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Description words length ( Set 0 to hide description. )', 'streamline' ),
				),
				'show_post_count' => array(
					...
					'options'		=> array(
						'show_post_count_check' => esc_html__( 'Show post count', 'streamline' ),
					),
				),
				...
				'first_item' => array(
					...
					'options'		=> array(
						'first_item_check' => esc_html__( 'Tiles first item', 'streamline' ),
					),
				),
				'items_padding' => array(
					'type'				=> 'stepper',
		...
		public function get_terms_list ( $tax = 'category', $args = array( 'hide_empty' => 0, 'hierarchical' => 0 ) ) {
			...
			if ( $terms ){
				foreach ( $terms as $term ) {
					$output_terms[ $term->term_id ] = $term->name . sprintf( _n( ' ( 1 post )', ' ( %s posts )', $term->count, 'streamline' ), $term->count );
				}
			}
		...
		public function get_term_post_count( $term, $visible ) {
			return ( 'false' === $visible ) ? '' : '<span class="post-count">' . sprintf( _n( '1 post', '%s posts', $term->count, 'streamline' ), $term->count ) . '</span>';
		}
		...
		public function get_term_description( $term, $lenght ) {
			...
			return ( 0 === $lenght || !$term->description ) ? '' : '<p class="post-desc">' . wp_trim_words( $term->description, $lenght, esc_html__( ' ...', 'streamline' ) ) . '</p>';
		}
		...
		public function get_term_image( $term, $image_size ) {
			$image = get_term_meta( $term->term_id, 'streamline_thumb' , true );
			...
			if( $image ){
				...
				$placeholder_attr = apply_filters( 'streamline_taxonomy_tiles_widget_placeholder_default_args',
					array(
						'width'			=> $size['width'],
						'height'		=> $size['height'],
						'background'	=> '000',
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( 'inc/widgets/tm-taxonomy-tiles-widget/views/tm-taxonomy-tiles-view.php', false, false );
			...
			if ( isset( $terms ) && $terms ) {
				...
				$tiles_item_number = apply_filters( 'streamline_taxonomy_tiles_item_number', 0 );
				...
				echo apply_filters( 'streamline_taxonomy_tiles_widget_before', '<div class="row grid-columns columns-number-' . $columns_number . '" ' . $row_inline_style . '>' );
				...
				foreach ( $terms as $term_key => $term ) {
					...
					$image = $this->get_term_image( $term, apply_filters( 'streamline_taxonomy_tiles_widget_size', 'streamline-thumb-426-327' ) );
				...
				echo apply_filters( 'streamline_taxonomy_tiles_widget_after', '</div>' );
			}
	...
	add_action( 'widgets_init', 'streamline_register_taxonomy_tiles_widget' );
	function streamline_register_taxonomy_tiles_widget() {
		register_widget( 'streamline_Taxonomy_Tiles_Widget' );
	}
