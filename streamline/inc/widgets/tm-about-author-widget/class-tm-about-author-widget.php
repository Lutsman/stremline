<?php
/**
 * Widget streamline about author
 *
 * @package streamline
 */

class streamline_About_Author_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'streamline widget-about-author';
		$this->widget_description = esc_html__( 'Display an information about selected user.', 'streamline' );
		$this->widget_id          = 'streamline_widget_about_author';
		$this->widget_name        = esc_html__( 'About Author', 'streamline' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'value' => esc_html__( 'About Author', 'streamline' ),
			...
			'user_id' => array(
				...
				'options'          => false,
				'label'            => esc_html__( 'Select user to show', 'streamline' ),
			),
			'avatar_size' => array(
				'type'       => 'slider',
				'max_value'  => 512,
				...
				'value'      => 250,
				'step_value' => 1,
				'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'streamline' ),
			),
			'avatar_img' => array(
				'type'               => 'media',
				'value'              => '',
				'multi_upload'       => false,
				'library_type'       => 'image',
				'upload_button_text' => esc_html__( 'Select image', 'streamline' ),
				'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'streamline' ),
			),
			'link' => array(
				'type'  => 'text',
				'value' => '',
				'label' => esc_html__( 'Link (leave empty to hide)', 'streamline' ),
			),
			'link_text' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Read More', 'streamline' ),
				'label' => esc_html__( 'Link label', 'streamline' ),
			),
		);
	...
	public function get_users_list() {
		...
		$result = array( '0' => esc_html__( 'Select a user', 'streamline' ) );
...
add_action( 'widgets_init', 'streamline_register_about_author_widgets' );
function streamline_register_about_author_widgets() {
	register_widget( 'streamline_About_Author_Widget' );
}
