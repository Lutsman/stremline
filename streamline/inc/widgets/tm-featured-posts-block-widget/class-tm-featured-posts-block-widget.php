...
if ( ! class_exists( 'streamline_Featured_Posts_Block_Widget' ) ) {

	/**
	 * Featured Posts Block Widget
	 */
	class streamline_Featured_Posts_Block_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Featured Posts Block', 'streamline' );
			$this->widget_description = esc_html__( 'This widget displays latest posts', 'streamline' );
			...
			foreach( $layouts as $layout_id => $layout_label ) {
				$layout_options[ $layout_id ] = array(
					'label' => $layout_label,
					'img_src' => STREAMLINE_THEME_URI . '/assets/images/admin/widgets/featured-posts-block/featured-posts-block-' . $layout_id . '.svg'
				);
			}
			...
			$this->settings = array(
				'layout'         => array(
					...
					'label'            => esc_html__( 'Layout', 'streamline' ),
					'options'          => $layout_options
				),
				'posts_ids' => array(
					'type'      => 'text',
					'value'     => '',
					'label'     => esc_html__( 'Posts IDs (Optional)', 'streamline' ),
				),
				'checkboxes'     => array(
					'type'  => 'checkbox',
					'label' => esc_html__( 'Post Meta', 'streamline' ),
					'value'   => array(
						'title'          => 'false',
						'excerpt'        => 'false',
						'categories'     => 'false',
					...
					'options' => array(
						'title'          => esc_html__( 'Show post title', 'streamline' ),
						'excerpt'        => esc_html__( 'Show post excerpt', 'streamline' ),
						'categories'     => esc_html__( 'Show post categories', 'streamline' ),
						'tags'           => esc_html__( 'Show post tags', 'streamline' ),
						'author'         => esc_html__( 'Show post author', 'streamline' ),
						'date'           => esc_html__( 'Show post date', 'streamline' ),
						'comments_count' => esc_html__( 'Show post comments count', 'streamline' ),
					),
				),
				'excerpt_length' => array(
					...
					'value'     => 55,
					'min_value' => 1,
					'label'     => esc_html__( 'Excerpt length', 'streamline' ),
				),
			);
		...
		public function get_layouts() {
			return array(
				'layout-1' => esc_html__( 'Layout #1', 'streamline' ),
				'layout-2' => esc_html__( 'Layout #2', 'streamline' ),
				'layout-3' => esc_html__( 'Layout #3', 'streamline' ),
				'layout-4' => esc_html__( 'Layout #4', 'streamline' ),
				'layout-5' => esc_html__( 'Layout #5', 'streamline' ),
			);
		}
		...
		public function post_date( array $args = array() ) {
			...
			if ( true === $for_human ) {
				$date = sprintf(
					_x( '%s ago', '%s ago - date when post was written', 'streamline' ),
	...
	add_action( 'widgets_init', 'streamline_register_featured_posts_block_widget' );

	if ( false === function_exists( 'streamline_register_featured_posts_block_widget' ) ) {
		function streamline_register_featured_posts_block_widget() {
			register_widget( 'streamline_Featured_Posts_Block_Widget' );
		}
	}
}
