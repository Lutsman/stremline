 ...
 * @return string
 */
function streamline_set_sidebar_position( $position ) {
	...
	$post_position = get_post_meta( $post_id, 'streamline_sidebar_position', true );
...
add_filter( 'theme_mod_sidebar_position', 'streamline_set_sidebar_position' );
 ...
 * @return string
 */
function streamline_render_macros( $string ) {

	$macros = apply_filters( 'streamline_data_macros', array(
		'/%%year%%/' => date( 'Y' ),
 ...
 * @return string
 */
function streamline_render_icons( $content ) {

	$icons     = streamline_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'streamline_render_icons_callback', $content );
}
 ...
 * @return string
 */
function streamline_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function streamline_get_render_icons_set() {
	return apply_filters( 'streamline_render_icons_set', array(
 ...
 * @return string
 */
function streamline_render_theme_url( $url ) {
